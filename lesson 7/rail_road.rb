class RailRoad
  ...
  def sub_menu_actions
    actions = { 0 => :menu_error, 1 => :add_station_to_route, 2 => :del_station_from_route, 3 => :assign_route_for_train, 4 => :add_wagon_to_train, 5 => :del_wagon_from_train, 6 => :move_train, 7 => :fill_wagon, 8 => :main_menu }
    ...
    loop do
      ...
      puts "Введите 7 чтобы заполнить вагон (в зависимости от типа вагона)"
  ...
  def create_route
    ...
    puts "\nСоздан маршрут #{from.name} - #{to.name}"
  ...
  def create_train
    ...
    print "Грузовой или пассажирский? (c/p): "
  ...
  def add_station_to_route
    ...
    display_route(route)
    print "\nВведите номер станции после которой нужно добавить: "
  ...
  def del_station_from_route
    ...
    display_route(route)
    print "\nВведите номер станции, который нужно удалить из маршрута: "
  ...
  def assign_route_for_train
    ...
    display_routes
    print "\nВведите номер маршрута, который нужно назначить поезду: "
    route = @routes[gets.to_i - 1]
  ...
  def add_wagon_to_train
    ...
    display_wagons
    print "\nВведите номер вагона, который нужно прицепить к поезду: "
  ...
  def del_wagon_from_train
    ...
    print "\nВведите номер вагона, который нужно отцепить от поезда: "
    wagon = @wagons[gets.to_i - 1]
  ...
  def move_train
    ...
    print "\nНа какую станцию нужно отправить поезд? (1 - на следующую, 2 - на предыдущую): "
    input = gets.to_i
  ...
  def fill_wagon
    ...
    print "\nВведите номер вагона, который нужно заполнить: "
    wagon = @wagons[gets.to_i - 1]

    if wagon.passenger?
      puts wagon.occupy_seat ? "\nВы заняли место в вагоне" : "\nСвободных мест в вагоне не осталось"
    elsif wagon.cargo?
      print "\Введите объем груза который нужно загрузить в вагон (м3): "
      volume = gets.to_f

      puts wagon.occupy_volume(volume) ? "\nГруз успешно загружен в вагон" : "\nНе достаточно места в вагоне!"
    end
  end
  ...
  def occupy_seat
    ...
    print "\nВведите номер вагона, в котором хотите занять место: "
  ...
  def display_wagons
    ...
    wagons.each_with_index do |wagon, i|
      if wagon.passenger?
        puts "#{i + 1}. номер: #{wagon.number}, тип: пассажирский,  мест занято: #{wagon.occupied_seats}, мест свободно: #{wagon.empty_seats}"
      elsif wagon.cargo?
        puts "#{i + 1}. номер: #{wagon.number}, тип: грузовой, грузом занято (м3): #{wagon.occupied_volume}, свободно (м3): #{wagon.empty_volume}" 
      end
    end
  end
  ...
  def display_train_wagons
    ...
    train.wagons do |wagon|
      if wagon.passenger?
        puts "#{i}. номер: #{wagon.number}, тип: пассажирский,  мест занято: #{wagon.occupied_seats}, мест свободно: #{wagon.empty_seats}"
      elsif wagon.cargo?
        puts "#{i}. номер: #{wagon.number}, тип: грузовой, грузом занято (м3): #{wagon.occupied_volume}, свободно (м3): #{wagon.empty_volume}" 
      end
      i += 1
    end
  end
  ...
  def display_routes
    puts "\nСписок маршрутов:"
    @routes.each_with_index { |route, i| puts "#{i + 1}. #{route.stations[0].name} - #{route.stations[-1].name}" }
  end
